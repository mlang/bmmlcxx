...
bool bmml::bow::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::bow::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
...
void bmml::bow::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::bow::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
...
bool bmml::clef::has_cross_staff() const {
  return attributes().find(qname("cross_staff")) != attributes().end();
}

bool bmml::clef::cross_staff() const {
  auto iter = attributes().find(qname("cross_staff"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
    throw illegal_enumeration{};
...
void bmml::clef::cross_staff(bool value) {
  attributes()[qname("cross_staff")] = value ? "true" : "false";
}

void bmml::clef::clear_cross_staff() {
  auto iter = attributes().find(qname("cross_staff"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::harmonic::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::harmonic::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::harmonic::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::harmonic::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::interval_type::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::interval_type::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::interval_type::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::interval_type::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::lyric_repetition::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::lyric_repetition::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::lyric_repetition::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::lyric_repetition::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::metronome_note_type::has_dot() const {
  return attributes().find(qname("dot")) != attributes().end();
}

bool bmml::metronome_note_type::dot() const {
  auto iter = attributes().find(qname("dot"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
    throw illegal_enumeration{};
...
void bmml::metronome_note_type::dot(bool value) {
  attributes()[qname("dot")] = value ? "true" : "false";
}

void bmml::metronome_note_type::clear_dot() {
  auto iter = attributes().find(qname("dot"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::nuance::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::nuance::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::nuance::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::nuance::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::ornament_type::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::ornament_type::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::ornament_type::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::ornament_type::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::pizzicato::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::pizzicato::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::pizzicato::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::pizzicato::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::string::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::string::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::string::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::string::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::stroke::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::stroke::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::stroke::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::stroke::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::syllabic_mute::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::syllabic_mute::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::syllabic_mute::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::syllabic_mute::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::time_signature::has_single_number() const {
  return attributes().find(qname("single_number")) != attributes().end();
}

bool bmml::time_signature::single_number() const {
  auto iter = attributes().find(qname("single_number"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
    throw illegal_enumeration{};
...
void bmml::time_signature::single_number(bool value) {
  attributes()[qname("single_number")] = value ? "true" : "false";
}

void bmml::time_signature::clear_single_number() {
  auto iter = attributes().find(qname("single_number"));
  if (iter != attributes().end()) attributes().erase(iter);
}

bool bmml::time_signature::has_figure() const {
  return attributes().find(qname("figure")) != attributes().end();
}

bool bmml::time_signature::figure() const {
  auto iter = attributes().find(qname("figure"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
    throw illegal_enumeration{};
...
void bmml::time_signature::figure(bool value) {
  attributes()[qname("figure")] = value ? "true" : "false";
}

void bmml::time_signature::clear_figure() {
  auto iter = attributes().find(qname("figure"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::tremolo::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::tremolo::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::tremolo::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::tremolo::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
...
bool bmml::tuplet::has_doubled() const {
  return attributes().find(qname("doubled")) != attributes().end();
}

bool bmml::tuplet::doubled() const {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) {
         if (iter->second == "true") return true;
    else if (iter->second == "false") return false;
...
void bmml::tuplet::doubled(bool value) {
  attributes()[qname("doubled")] = value ? "true" : "false";
}

void bmml::tuplet::clear_doubled() {
  auto iter = attributes().find(qname("doubled"));
  if (iter != attributes().end()) attributes().erase(iter);
}
